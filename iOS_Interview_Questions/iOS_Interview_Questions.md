...
// -----------------------------------------------------------------------------
// Phantom-type state markers (empty types used only at compile time)
// -----------------------------------------------------------------------------
struct LoggedOut {}
struct LoggedIn {}

// -----------------------------------------------------------------------------
// SESSION example: enforce login-only actions at compile time
// -----------------------------------------------------------------------------
struct Session<State> {
    // underlying stored data (exists at runtime)
...
// Actions allowed only when Session is LoggedOut
extension Session where State == LoggedOut {
    /// Attempt login (simulated). Returns a Session<LoggedIn> if successful.
    func login(username: String, password: String) -> Session<LoggedIn> {
        // (In real app you'd call the network and obtain a token)
        print("[Session<LoggedOut>] Logging in user: \(username)")
        let simulatedToken = "token_for_\(username)"
        return Session<LoggedIn>(token: simulatedToken)
    }
}

// Actions allowed only when Session is LoggedIn
extension Session where State == LoggedIn {
    /// Access a protected resource — allowed only for logged-in sessions
    func getProtectedResource() {
        print("[Session<LoggedIn>] Accessing protected resource with token: \(token ?? "<none>")")
    }

    /// Log out: transition back to LoggedOut
    func logout() -> Session<LoggedOut> {
        print("[Session<LoggedIn>] Logging out")
        return Session<LoggedOut>(token: nil)
    }
}

// -----------------------------------------------------------------------------
// PAYMENT example: enforce authentication before completing payment
// -----------------------------------------------------------------------------
struct Created {}
...
extension Payment where State == Created {
    func authenticate(userToken: String) -> Payment<Authenticated> {
        print("[Payment<Created>] Authenticating payment of \(amount) with userToken: \(userToken)")
        return Payment<Authenticated>(amount: amount)
    }
}

extension Payment where State == Authenticated {
    func complete() -> Payment<Completed> {
        print("[Payment<Authenticated>] Completing payment of \(amount)")
        return Payment<Completed>(amount: amount)
    }
}

// -----------------------------------------------------------------------------
// SECURE RESOURCE example: only allow accessing secret data when Unlocked
// -----------------------------------------------------------------------------
struct Locked {}
struct Unlocked {}

struct SecureResource<State> {
    // secret exists at runtime but access is gated by the phantom type
...
extension SecureResource where State == Locked {
    func unlock(withAuth: Bool) -> SecureResource<Unlocked>? {
        print("[SecureResource<Locked>] Attempting unlock...")
        guard withAuth else {
            print("[SecureResource<Locked>] Unlock failed: authentication missing")
            return nil
        }
        print("[SecureResource<Locked>] Unlock successful")
        return SecureResource<Unlocked>(secret: secret)
    }
}

extension SecureResource where State == Unlocked {
    func read() -> String {
        print("[SecureResource<Unlocked>] Reading secret")
        return secret
    }
}

// -----------------------------------------------------------------------------
// Demo runner: show valid usage and commented compile-time errors
// -----------------------------------------------------------------------------
func demoSessionFlow() {
    print("--- demoSessionFlow ---")

    // Start with a logged-out session
    let loggedOut = Session<LoggedOut>(token: nil)

    // This would be a compile-time error (uncomment to see it fail):
    // loggedOut.getProtectedResource() // ❌ Not allowed

    // Login -> returns Session<LoggedIn>
    let loggedIn = loggedOut.login(username: "alice", password: "secret")

    // Now allowed:
    loggedIn.getProtectedResource()

    // Logout -> back to logged-out
    let backToLoggedOut = loggedIn.logout()
    _ = backToLoggedOut

    print()
}

func demoPaymentFlow() {
    print("--- demoPaymentFlow ---")
...
func demoSecureResourceFlow() {
    print("--- demoSecureResourceFlow ---")
    ...
    // Unlock with auth and read
    if let unlocked = lockedResource.unlock(withAuth: true) {
        let secret = unlocked.read()
        print("Secret: \(secret)")
    }
